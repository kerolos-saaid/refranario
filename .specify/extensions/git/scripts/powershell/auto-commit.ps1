...
# Stage and commit. Keep Spec Kit automation scoped so unrelated workspace
# artifacts (for example local tmp/media/import folders) are not swept into
# automatic commits.
# Relax ErrorActionPreference so CRLF warnings on stderr do not terminate,
...
try {
    ...
    $out = git add -- $specKitPaths 2>&1 | Out-String
